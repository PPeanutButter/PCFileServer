...
class FocusRowBehaviorDelegate{constructor(listItem){this.listItem_=listItem}onFocus(row,e){const element=e.path[0];const focusableElement=FocusRow.getFocusableElement(element);if(element!==focusableElement){focusableElement.focus()}this.listItem_.lastFocused=focusableElement}onKeydown(row,e){if(e.key==="Enter"){e.stopPropagation()}return false}getCustomEquivalent(sampleElement){return this.listItem_.overrideCustomEquivalent?this.listItem_.getCustomEquivalent(sampleElement):null}}class VirtualFocusRow extends FocusRow{constructor(root,delegate){super(root,null,delegate)}getCustomEquivalent(sampleElement){return this.delegate.getCustomEquivalent(sampleElement)||super.getCustomEquivalent(sampleElement)}}const FocusRowBehavior={properties:{row_:Object,mouseFocused_:Boolean,id:{type:String,reflectToAttribute:true},isFocused:{type:Boolean,notify:true},focusRowIndex:{type:Number,observer:"focusRowIndexChanged"},lastFocused:{type:Object,notify:true},ironListTabIndex:{type:Number,observer:"ironListTabIndexChanged_"},listBlurred:{type:Boolean,notify:true}},computeId_(index){return index!==undefined?`frb${index}`:undefined},focusRowIndexChanged(newIndex,oldIndex){this.setAttribute("aria-rowindex",newIndex+1);if(this.id===this.computeId_(oldIndex)){this.id=this.computeId_(newIndex)}},firstControl_:null,controlObservers_:[],attached(){this.classList.add("no-outline");afterNextRender(this,(function(){const rowContainer=this.root.querySelector("[focus-row-container]");assert(rowContainer);this.row_=new VirtualFocusRow(rowContainer,new FocusRowBehaviorDelegate(this));this.addItems_();this.listen(this,"focus","onFocus_");this.listen(this,"dom-change","addItems_");this.listen(this,"mousedown","onMouseDown_");this.listen(this,"blur","onBlur_")}))},detached(){this.unlisten(this,"focus","onFocus_");this.unlisten(this,"dom-change","addItems_");this.unlisten(this,"mousedown","onMouseDown_");this.unlisten(this,"blur","onBlur_");this.removeObservers_();if(this.firstControl_){this.unlisten(this.firstControl_,"keydown","onFirstControlKeydown_")}if(this.row_){this.row_.destroy()}},getFocusRow(){return assert(this.row_)},updateFirstControl_(){const newFirstControl=this.row_.getFirstFocusable();if(newFirstControl===this.firstControl_){return}if(this.firstControl_){this.unlisten(this.firstControl_,"keydown","onFirstControlKeydown_")}this.firstControl_=newFirstControl;if(this.firstControl_){this.listen(this.firstControl_,"keydown","onFirstControlKeydown_")}},removeObservers_(){if(this.controlObservers_.length>0){this.controlObservers_.forEach((observer=>{observer.disconnect()}))}this.controlObservers_=[]},addItems_(){this.ironListTabIndexChanged_();if(this.row_){this.removeObservers_();this.row_.destroy();const controls=this.root.querySelectorAll("[focus-row-control]");controls.forEach((control=>{this.row_.addItem(control.getAttribute("focus-type"),FocusRow.getFocusableElement(control));this.addMutationObservers_(assert(control))}));this.updateFirstControl_()}},createObserver_(){return new MutationObserver((mutations=>{const mutation=mutations[0];if(mutation.attributeName==="style"&&mutation.oldValue){const newStyle=window.getComputedStyle(mutation.target);const oldDisplayValue=mutation.oldValue.match(/^display:(.*)(?=;)/);const oldVisibilityValue=mutation.oldValue.match(/^visibility:(.*)(?=;)/);if(oldDisplayValue&&newStyle.display===oldDisplayValue[1].trim()&&oldVisibilityValue&&newStyle.visibility===oldVisibilityValue[1].trim()){return}}this.updateFirstControl_()}))},addMutationObservers_(control){let current=control;while(current&&current!==this.root){const currentObserver=this.createObserver_();currentObserver.observe(current,{attributes:true,attributeFilter:["hidden","disabled","style"],attributeOldValue:true});this.controlObservers_.push(currentObserver);current=current.parentNode}},onFocus_(e){if(this.mouseFocused_){this.mouseFocused_=false;return}const restoreFocusToFirst=this.listBlurred&&e.composedPath()[0]===this;if(this.lastFocused&&!restoreFocusToFirst){focusWithoutInk(this.row_.getEquivalentElement(this.lastFocused))}else{const firstFocusable=assert(this.firstControl_);focusWithoutInk(firstFocusable)}this.listBlurred=false;this.isFocused=true},onFirstControlKeydown_(e){if(e.shiftKey&&e.key==="Tab"){this.focus()}},ironListTabIndexChanged_(){if(this.row_){this.row_.makeActive(this.ironListTabIndex===0)}if(this.ironListTabIndex===0){this.listBlurred=false}},onMouseDown_(){this.mouseFocused_=true},onBlur_(e){this.mouseFocused_=false;this.isFocused=false;const node=e.relatedTarget?e.relatedTarget:null;if(!this.parentNode.contains(node)){this.listBlurred=true}}};const PageHandlerFactoryPendingReceiver=class{constructor(handle){this.handle=mojo.internal.interfaceSupport.getEndpointForReceiver(handle)}bindInBrowser(scope="context"){mojo.internal.interfaceSupport.bind(this.handle,"downloads.mojom.PageHandlerFactory",scope)}};const PageHandlerFactoryRemote=class{constructor(handle=undefined){this.proxy=new mojo.internal.interfaceSupport.InterfaceRemoteBase(PageHandlerFactoryPendingReceiver,handle);this.$=new mojo.internal.interfaceSupport.InterfaceRemoteBaseWrapper(this.proxy);this.onConnectionError=this.proxy.getConnectionErrorEventRouter()}createPageHandler(page,handler){this.proxy.sendMessage(211959741,PageHandlerFactory_CreatePageHandler_ParamsSpec.$,null,[page,handler])}};const PageHandlerFactory=class{static get $interfaceName(){return"downloads.mojom.PageHandlerFactory"}static getRemote(){let remote=new PageHandlerFactoryRemote;remote.$.bindNewPipeAndPassReceiver().bindInBrowser();return remote}};const PageHandlerPendingReceiver=class{constructor(handle){this.handle=mojo.internal.interfaceSupport.getEndpointForReceiver(handle)}bindInBrowser(scope="context"){mojo.internal.interfaceSupport.bind(this.handle,"downloads.mojom.PageHandler",scope)}};const PageHandlerInterface=class{getDownloads(searchTerms){}openFileRequiringGesture(id){}drag(id){}saveDangerousRequiringGesture(id){}discardDangerous(id){}retryDownload(id){}show(id){}pause(id){}resume(id){}remove(id){}undo(){}cancel(id){}clearAll(){}openDownloadsFolderRequiringGesture(){}openDuringScanningRequiringGesture(id){}};const PageHandlerRemote=class{constructor(handle=undefined){this.proxy=new mojo.internal.interfaceSupport.InterfaceRemoteBase(PageHandlerPendingReceiver,handle);this.$=new mojo.internal.interfaceSupport.InterfaceRemoteBaseWrapper(this.proxy);this.onConnectionError=this.proxy.getConnectionErrorEventRouter()}getDownloads(searchTerms){this.proxy.sendMessage(598058465,PageHandler_GetDownloads_ParamsSpec.$,null,[searchTerms])}openFileRequiringGesture(id){this.proxy.sendMessage(1712284511,PageHandler_OpenFileRequiringGesture_ParamsSpec.$,null,[id])}drag(id){this.proxy.sendMessage(1926310623,PageHandler_Drag_ParamsSpec.$,null,[id])}saveDangerousRequiringGesture(id){this.proxy.sendMessage(2096757854,PageHandler_SaveDangerousRequiringGesture_ParamsSpec.$,null,[id])}discardDangerous(id){this.proxy.sendMessage(1216892833,PageHandler_DiscardDangerous_ParamsSpec.$,null,[id])}retryDownload(id){this.proxy.sendMessage(1596564378,PageHandler_RetryDownload_ParamsSpec.$,null,[id])}show(id){this.proxy.sendMessage(987363029,PageHandler_Show_ParamsSpec.$,null,[id])}pause(id){this.proxy.sendMessage(1761287570,PageHandler_Pause_ParamsSpec.$,null,[id])}resume(id){this.proxy.sendMessage(506239065,PageHandler_Resume_ParamsSpec.$,null,[id])}remove(id){this.proxy.sendMessage(794779134,PageHandler_Remove_ParamsSpec.$,null,[id])}undo(){this.proxy.sendMessage(673644695,PageHandler_Undo_ParamsSpec.$,null,[])}cancel(id){this.proxy.sendMessage(368821124,PageHandler_Cancel_ParamsSpec.$,null,[id])}clearAll(){this.proxy.sendMessage(496493960,PageHandler_ClearAll_ParamsSpec.$,null,[])}openDownloadsFolderRequiringGesture(){this.proxy.sendMessage(948716775,PageHandler_OpenDownloadsFolderRequiringGesture_ParamsSpec.$,null,[])}openDuringScanningRequiringGesture(id){this.proxy.sendMessage(2109245274,PageHandler_OpenDuringScanningRequiringGesture_ParamsSpec.$,null,[id])}};const PagePendingReceiver=class{constructor(handle){this.handle=mojo.internal.interfaceSupport.getEndpointForReceiver(handle)}bindInBrowser(scope="context"){mojo.internal.interfaceSupport.bind(this.handle,"downloads.mojom.Page",scope)}};const PageInterface=class{removeItem(index){}updateItem(index,data){}insertItems(index,items){}clearAll(){}};const PageRemote=class{constructor(handle=undefined){this.proxy=new mojo.internal.interfaceSupport.InterfaceRemoteBase(PagePendingReceiver,handle);this.$=new mojo.internal.interfaceSupport.InterfaceRemoteBaseWrapper(this.proxy);this.onConnectionError=this.proxy.getConnectionErrorEventRouter()}removeItem(index){this.proxy.sendMessage(1351388799,Page_RemoveItem_ParamsSpec.$,null,[index])}updateItem(index,data){this.proxy.sendMessage(1150376170,Page_UpdateItem_ParamsSpec.$,null,[index,data])}insertItems(index,items){this.proxy.sendMessage(1425336901,Page_InsertItems_ParamsSpec.$,null,[index,items])}clearAll(){this.proxy.sendMessage(1823971443,Page_ClearAll_ParamsSpec.$,null,[])}};const PageCallbackRouter=class{constructor(){this.helper_internal_=new mojo.internal.interfaceSupport.InterfaceReceiverHelperInternal(PageRemote);this.$=new mojo.internal.interfaceSupport.InterfaceReceiverHelper(this.helper_internal_);this.router_=new mojo.internal.interfaceSupport.CallbackRouter;this.removeItem=new mojo.internal.interfaceSupport.InterfaceCallbackReceiver(this.router_);this.helper_internal_.registerHandler(1351388799,Page_RemoveItem_ParamsSpec.$,null,this.removeItem.createReceiverHandler(false));this.updateItem=new mojo.internal.interfaceSupport.InterfaceCallbackReceiver(this.router_);this.helper_internal_.registerHandler(1150376170,Page_UpdateItem_ParamsSpec.$,null,this.updateItem.createReceiverHandler(false));this.insertItems=new mojo.internal.interfaceSupport.InterfaceCallbackReceiver(this.router_);this.helper_internal_.registerHandler(1425336901,Page_InsertItems_ParamsSpec.$,null,this.insertItems.createReceiverHandler(false));this.clearAll=new mojo.internal.interfaceSupport.InterfaceCallbackReceiver(this.router_);this.helper_internal_.registerHandler(1823971443,Page_ClearAll_ParamsSpec.$,null,this.clearAll.createReceiverHandler(false));this.onConnectionError=this.helper_internal_.getConnectionErrorEventRouter()}removeListener(id){return this.router_.removeListener(id)}};const DataSpec={$:{}};const PageHandlerFactory_CreatePageHandler_ParamsSpec={$:{}};const PageHandler_GetDownloads_ParamsSpec={$:{}};const PageHandler_OpenFileRequiringGesture_ParamsSpec={$:{}};const PageHandler_Drag_ParamsSpec={$:{}};const PageHandler_SaveDangerousRequiringGesture_ParamsSpec={$:{}};const PageHandler_DiscardDangerous_ParamsSpec={$:{}};const PageHandler_RetryDownload_ParamsSpec={$:{}};const PageHandler_Show_ParamsSpec={$:{}};const PageHandler_Pause_ParamsSpec={$:{}};const PageHandler_Resume_ParamsSpec={$:{}};const PageHandler_Remove_ParamsSpec={$:{}};const PageHandler_Undo_ParamsSpec={$:{}};const PageHandler_Cancel_ParamsSpec={$:{}};const PageHandler_ClearAll_ParamsSpec={$:{}};const PageHandler_OpenDownloadsFolderRequiringGesture_ParamsSpec={$:{}};const PageHandler_OpenDuringScanningRequiringGesture_ParamsSpec={$:{}};const Page_RemoveItem_ParamsSpec={$:{}};const Page_UpdateItem_ParamsSpec={$:{}};const Page_InsertItems_ParamsSpec={$:{}};const Page_ClearAll_ParamsSpec={$:{}};mojo.internal.Struct(DataSpec.$,"Data",[mojo.internal.StructField("fileExternallyRemoved",0,0,mojo.internal.Bool,false,false,0),mojo.internal.StructField("isDangerous",0,1,mojo.internal.Bool,false,false,0),mojo.internal.StructField("isMixedContent",0,2,mojo.internal.Bool,false,false,0),mojo.internal.StructField("otr",0,3,mojo.internal.Bool,false,false,0),mojo.internal.StructField("resume",0,4,mojo.internal.Bool,false,false,0),mojo.internal.StructField("retry",0,5,mojo.internal.Bool,false,false,0),mojo.internal.StructField("percent",4,0,mojo.internal.Int32,0,false,0),mojo.internal.StructField("started",8,0,mojo.internal.Int32,0,false,0),mojo.internal.StructField("total",12,0,mojo.internal.Int32,0,false,0),mojo.internal.StructField("byExtId",16,0,mojo.internal.String,null,false,0),mojo.internal.StructField("byExtName",24,0,mojo.internal.String,null,false,0),mojo.internal.StructField("dangerType",32,0,mojo.internal.String,null,false,0),mojo.internal.StructField("dateString",40,0,mojo.internal.String,null,false,0),mojo.internal.StructField("fileName",48,0,mojo.internal.String,null,false,0),mojo.internal.StructField("filePath",56,0,mojo.internal.String,null,false,0),mojo.internal.StructField("fileUrl",64,0,mojo.internal.String,null,false,0),mojo.internal.StructField("id",72,0,mojo.internal.String,null,false,0),mojo.internal.StructField("lastReasonText",80,0,mojo.internal.String,null,false,0),mojo.internal.StructField("progressStatusText",88,0,mojo.internal.String,null,false,0),mojo.internal.StructField("sinceString",96,0,mojo.internal.String,null,false,0),mojo.internal.StructField("state",104,0,mojo.internal.String,null,false,0),mojo.internal.StructField("url",112,0,mojo.internal.String,null,false,0)],[[0,128]]);mojo.internal.Struct(PageHandlerFactory_CreatePageHandler_ParamsSpec.$,"PageHandlerFactory_CreatePageHandler_Params",[mojo.internal.StructField("page",0,0,mojo.internal.InterfaceProxy(PageRemote),null,false,0),mojo.internal.StructField("handler",8,0,mojo.internal.InterfaceRequest(PageHandlerPendingReceiver),null,false,0)],[[0,24]]);mojo.internal.Struct(PageHandler_GetDownloads_ParamsSpec.$,"PageHandler_GetDownloads_Params",[mojo.internal.StructField("searchTerms",0,0,mojo.internal.Array(mojo.internal.String,false),null,false,0)],[[0,16]]);mojo.internal.Struct(PageHandler_OpenFileRequiringGesture_ParamsSpec.$,"PageHandler_OpenFileRequiringGesture_Params",[mojo.internal.StructField("id",0,0,mojo.internal.String,null,false,0)],[[0,16]]);mojo.internal.Struct(PageHandler_Drag_ParamsSpec.$,"PageHandler_Drag_Params",[mojo.internal.StructField("id",0,0,mojo.internal.String,null,false,0)],[[0,16]]);mojo.internal.Struct(PageHandler_SaveDangerousRequiringGesture_ParamsSpec.$,"PageHandler_SaveDangerousRequiringGesture_Params",[mojo.internal.StructField("id",0,0,mojo.internal.String,null,false,0)],[[0,16]]);mojo.internal.Struct(PageHandler_DiscardDangerous_ParamsSpec.$,"PageHandler_DiscardDangerous_Params",[mojo.internal.StructField("id",0,0,mojo.internal.String,null,false,0)],[[0,16]]);mojo.internal.Struct(PageHandler_RetryDownload_ParamsSpec.$,"PageHandler_RetryDownload_Params",[mojo.internal.StructField("id",0,0,mojo.internal.String,null,false,0)],[[0,16]]);mojo.internal.Struct(PageHandler_Show_ParamsSpec.$,"PageHandler_Show_Params",[mojo.internal.StructField("id",0,0,mojo.internal.String,null,false,0)],[[0,16]]);mojo.internal.Struct(PageHandler_Pause_ParamsSpec.$,"PageHandler_Pause_Params",[mojo.internal.StructField("id",0,0,mojo.internal.String,null,false,0)],[[0,16]]);mojo.internal.Struct(PageHandler_Resume_ParamsSpec.$,"PageHandler_Resume_Params",[mojo.internal.StructField("id",0,0,mojo.internal.String,null,false,0)],[[0,16]]);mojo.internal.Struct(PageHandler_Remove_ParamsSpec.$,"PageHandler_Remove_Params",[mojo.internal.StructField("id",0,0,mojo.internal.String,null,false,0)],[[0,16]]);mojo.internal.Struct(PageHandler_Undo_ParamsSpec.$,"PageHandler_Undo_Params",[],[[0,8]]);mojo.internal.Struct(PageHandler_Cancel_ParamsSpec.$,"PageHandler_Cancel_Params",[mojo.internal.StructField("id",0,0,mojo.internal.String,null,false,0)],[[0,16]]);mojo.internal.Struct(PageHandler_ClearAll_ParamsSpec.$,"PageHandler_ClearAll_Params",[],[[0,8]]);mojo.internal.Struct(PageHandler_OpenDownloadsFolderRequiringGesture_ParamsSpec.$,"PageHandler_OpenDownloadsFolderRequiringGesture_Params",[],[[0,8]]);mojo.internal.Struct(PageHandler_OpenDuringScanningRequiringGesture_ParamsSpec.$,"PageHandler_OpenDuringScanningRequiringGesture_Params",[mojo.internal.StructField("id",0,0,mojo.internal.String,null,false,0)],[[0,16]]);mojo.internal.Struct(Page_RemoveItem_ParamsSpec.$,"Page_RemoveItem_Params",[mojo.internal.StructField("index",0,0,mojo.internal.Int32,0,false,0)],[[0,16]]);mojo.internal.Struct(Page_UpdateItem_ParamsSpec.$,"Page_UpdateItem_Params",[mojo.internal.StructField("index",0,0,mojo.internal.Int32,0,false,0),mojo.internal.StructField("data",8,0,DataSpec.$,null,false,0)],[[0,24]]);mojo.internal.Struct(Page_InsertItems_ParamsSpec.$,"Page_InsertItems_Params",[mojo.internal.StructField("index",0,0,mojo.internal.Int32,0,false,0),mojo.internal.StructField("items",8,0,mojo.internal.Array(DataSpec.$,false),null,false,0)],[[0,24]]);mojo.internal.Struct(Page_ClearAll_ParamsSpec.$,"Page_ClearAll_Params",[],[[0,8]]);// Copyright 2017 The Chromium Authors. All rights reserved.
...
<div id="content" on-dragstart="onDragStart_" class$="[[computeClass_(isActive_, isDangerous_, showProgress_)]]" focus-row-container="">
  ...
  <div id="details">
    ...
    <div id="safe" class="controls" hidden="[[isDangerous_]]">
      <div role="gridcell">
        <a is="action-link" id="show" on-click="onShowTap_" hidden="[[!completelyOnDisk_]]" focus-row-control="" focus-type="show">在文件夹中显示</a>
      </div>
    ...
    <template is="dom-if" if="[[isDangerous_]]">
      <div id="dangerous" class="controls">
        ...
        <template is="dom-if" if="[[!isMalware_]]">
          <div role="gridcell">
            <cr-button on-click="onDiscardDangerousTap_" class="action-button" focus-row-control="" focus-type="discard">舍弃</cr-button>
          </div>
          <div role="gridcell">
  ...
  <div role="gridcell">
    <cr-icon-button class="icon-clear" style$="[[computeRemoveStyle_(isDangerous_, showCancel_)]]" id="remove" title="从列表中移除" aria-label$="[[controlRemoveFromListAriaLabel_]]" on-click="onRemoveTap_" focus-row-control="" focus-type="remove">
    </cr-icon-button>
  </div>
  <div id="incognito" title="已在隐身模式下进行下载" hidden="[[!data.otr]]">
  </div>
</div>
...
<cr-toolbar id="toolbar" page-name="下载内容" autofocus="" search-prompt="搜索下载内容" clear-label="清除搜索字词" spinner-active="{{spinnerActive}}" on-search-changed="onSearchChanged_">
  <cr-icon-button id="moreActions" iron-icon="cr:more-vert" class="dropdown-trigger" title="更多操作" on-click="onMoreActionsTap_" aria-haspopup="menu"></cr-icon-button>
</cr-toolbar>
<cr-action-menu id="moreActionsMenu" role-description="操作">
...
<cr-toast-manager duration="10000">
  <cr-button aria-label="按 Ctrl+Z 即可撤消" on-click="onUndoClick_">
    撤消
  </cr-button>
</cr-toast-manager>
